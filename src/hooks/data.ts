...
export function useAxes({
  ...
  useEffect(() => {
    const xAxis = getAxisDefinition(
      rows.map((d) => data.columns[d]),
      data
    );
    const yAxis = getAxisDefinition(
      columns.map((d) => data.columns[d]),
      data
    );
    setX(xAxis);
    setY(yAxis);
    ...
    const finalData: CellData[] = data.rows
      .filter((row) => {
        ...
        filters.forEach((filter) => {
          if (filter.field && row[filter.field] !== filter.value)
            passes = false;
        });
        return passes;
      })
